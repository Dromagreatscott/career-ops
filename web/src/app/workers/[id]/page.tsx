...
export default function WorkerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!job) {
    return (
      <div className="mx-auto max-w-3xl px-6 py-10">
        <Link href="/workers" className="inline-flex items-center gap-1.5 text-sm text-muted transition-colors hover:text-brand">
          <ArrowLeft className="size-4" /> Workers
        </Link>
        <p className="mt-8 text-sm text-muted">This worker is no longer in memory.</p>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-8">
      ...
      <section className="dot-bg relative mt-5 overflow-hidden rounded-xl border border-border bg-surface/40 px-6 py-7">
      ...
      {job.text && (
        <div className="mt-8">
          ...
          <div className="report-prose mt-3 rounded-xl border border-border bg-surface/40 p-5">
