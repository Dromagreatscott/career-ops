...
export default function WorkersHistory() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-10">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            Every evaluation you ran. <span className="tabular-nums">{jobs.length}</span> total.
          </p>
        </div>
      ...
      {jobs.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed border-border bg-surface/30 px-6 py-12 text-center text-sm text-muted">
          No workers yet. Score a shortlist from the pipeline to spin one up.
        </div>
      ) : (
        <ul className="mt-6 divide-y divide-border overflow-hidden rounded-xl border border-border bg-surface/40">
          {jobs.map((j) => {
            const tone = pillTone(j);
            return (
              <li key={j.id}>
                <Link href={`/workers/${j.id}`} className="flex items-center gap-3 px-4 py-3 transition-colors hover:bg-surface-hover">
                  {j.status === "running" ? (
